 ...
 * EVN CPC / NPC / Hanoi: Chromium chỉ launch khi có task PENDING được claim; sau khi mọi task đang chạy xong
...
export class TaskRunner {
  ...
  private readonly hanoiWorker = new EVNHanoiWorker(this.captcha);
      ...
      const results = await Promise.all(
        Array.from({ length: env.workerConcurrency }, () =>
          limit(async (): Promise<boolean> => {
            try {
              return await claimAndProcessNext(
                this.repo,
                this.evnWorker,
                this.npcWorker,
                this.workerId,
